...
def main():
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        for e in range(num_scenes):
            ...
            # choose global goal map
            local_ob_map[e] = cv2.dilate(full_map[e][0].cpu().numpy(), kernel)

            show_ex = cv2.inRange(full_map[e][1].cpu().numpy(),0.1,1)
            ...
            # clear the boundary
            local_ex_map[e, 0:2, 0:full_w]=0.0
            local_ex_map[e, full_w-2:full_w, 0:full_w-1]=0.0
            local_ex_map[e, 0:full_w, 0:2]=0.0
            local_ex_map[e, 0:full_w, full_w-2:full_w]=0.0
            

            target_lo_map[e] = local_ex_map[e]-local_ob_map[e]

            target_lo_map[e][target_lo_map[e]>0.8]=1.0
            target_lo_map[e][target_lo_map[e]!=1.0]=0.0
            
            full_pose_map = [full_pose[e][1]*100/args.map_resolution, full_pose[e][0]*100/args.map_resolution]
            target_lo_map[e] = remove_small_points(target_lo_map[e], 4) 
    
            target_lo_map[e, int(full_pose_map[0])-25:int(full_pose_map[0])+25, int(full_pose_map[1])-25:int(full_pose_map[1])+25]=0.0
        ...
        for e in range(num_scenes):
            ...
            if target_lo_map[e].sum()!=0.:
                full_goal_maps[e] = target_lo_map[e]
