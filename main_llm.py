...
def main():
    ...
    g_sum_rewards = np.zeros((num_scenes))
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                for lay in range(tpm):
                    ...
                    if len(objs_list)>0:
                        ...
                        if args.task_config == "tasks/objectnav_hm3d.yaml": 
                            frontier_score_list[e].append(pred[0][hm3d_category.index(cname)]) 
                        else:
                            frontier_score_list[e].append(pred[0][category_to_id_gibson.index(cname)]) 
                    else:
                        frontier_score_list[e].append(0.0001) 
            ...
            # g_sum_rewards += g_total_rewards
            # g_total_rewards = g_process_rewards * \
            #     (1 - g_masks.cpu().numpy())
            # g_process_rewards *= g_masks.cpu().numpy()
            # per_step_g_rewards.append(np.mean(g_reward.cpu().numpy()))
            # print("LLM success rate: ", g_process_rewards.sum() /g_sum_rewards.sum())
        ...
        for e in range(num_scenes):
            ...
            # ------------------------------------------------------------------
            global_item = 0
            if len(frontier_score_list[e]) > 0 and max(frontier_score_list[e]) > 0.3:
        ...
        if step % args.log_interval == 0:
            ...
            # log += "\n\tLLM Rewards: " + str(g_process_rewards.sum() /g_sum_rewards.sum())
